...
const Footer = () => {
  ...
  const handleNewsletter = async (e: React.FormEvent) => {
    ...
    if (!email.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      setMsg({ type:"error", text:"Veuillez entrer un email valide" }); return;
    }
    setLoading(true); setMsg(null);
    try {
      await emailjs.send(SERVICE_ID, TEMPLATE_ID, { to_email: email, subject:"Bienvenue à la Newsletter Bridge Partners", message:"Merci de vous être abonné !" }, USER_ID);
      setMsg({ type:"success", text:"Abonnement réussi ! Merci !" });
      setEmail("");
    } catch { setMsg({ type:"error", text:"Erreur lors de l'abonnement. Réessayez." }); }
    finally { setLoading(false); }
  };

  const navCols = [
    { title:"Navigation", links:[{l:"Accueil",to:"/"},{l:"À propos",to:"/about"},{l:"Articles",to:"/articles"},{l:"Découvrir le Bénin",to:"/benin"},{l:"Nos Services",to:"/team"},{l:"Contact",to:"/contact"}] },
  ];
  ...
  return (
    <footer className="bg-[#040f2e] relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
        ...
        <div>
          <Link to="/" className="flex items-center gap-3 mb-4">
            ...
            <div className="flex flex-col leading-none">
              ...
              <span className="text-[8px] tracking-[.16em] uppercase text-[#c9a84c]">Connecter des ambitions</span>
            </div>
          </Link>
        ...
        {/* Navigation */}
        <div>
          <h4 className="text-[10px] font-semibold uppercase tracking-[.18em] text-[#c9a84c] mb-4">Navigation</h4>
          <ul className="space-y-2">
            {navCols[0].links.map(({l,to})=>(
              <li key={l}><Link to={to} className="text-white/45 text-sm hover:text-white transition-colors duration-200">{l}</Link></li>
            ))}
          </ul>
        </div>
        ...
        {/* Coordonnées */}
        <div>
          <h4 className="text-[10px] font-semibold uppercase tracking-[.18em] text-[#c9a84c] mb-4">Coordonnées</h4>
          {[
            ...
            <div key={val} className="flex gap-2 mb-3 items-start">
              <span className="text-[#c9a84c] text-sm flex-shrink-0 mt-0.5">{ico}</span>
        ...
        {/* Newsletter */}
        <div>
          <h4 className="text-[10px] font-semibold uppercase tracking-[.18em] text-[#c9a84c] mb-4">Newsletter</h4>
          ...
          <form onSubmit={handleNewsletter} className="flex flex-col gap-2">
            <input type="email" placeholder="Votre adresse e-mail" value={email}
              ...
              className="px-3 py-2.5 rounded-lg bg-white/06 border border-white/10 text-white text-xs placeholder:text-white/30 focus:outline-none focus:border-[#c9a84c] transition disabled:opacity-50" />
            <button type="submit" disabled={loading}
              className="py-2.5 bg-[#c9a84c] text-[#040f2e] text-xs font-bold uppercase tracking-[.1em] rounded-lg hover:bg-[#f0d896] transition disabled:opacity-50">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex flex-col md:flex-row justify-between items-center gap-3">
          ...
          <div className="flex gap-3">
            {[FaLinkedin, FaInstagram, FaFacebook, FaTiktok].map((Icon, i) => (
              <a key={i} href="#"
                className="w-8 h-8 border border-white/10 rounded-md flex items-center justify-center text-white/35 text-sm hover:border-[#c9a84c] hover:text-[#c9a84c] transition-all duration-200">
                <Icon />
              </a>
            ))}
          </div>
